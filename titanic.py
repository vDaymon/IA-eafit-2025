...
# ============ 1) Cargar datos ============
CSV_PATH = r"C:\Users\USUARIO\OneDrive\Documentos\TrabajoIA\titanic\train.csv"  # <-- ajusta si es necesario
...
# Escalado (beneficia a la Red Neuronal; al RF no le afecta)
scaler = StandardScaler()
X_train_s = scaler.fit_transform(X_train)
X_test_s  = scaler.transform(X_test)

# ============ 4) Modelos ============
# Random Forest (igual que tenías; puedes ajustar n_estimators si deseas)
rf = RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=-1)

# Red Neuronal (MLPClassifier)
# - early_stopping: previene sobreajuste
# - hidden_layer_sizes: dos capas (64, 32) suele ir bien en este dataset
mlp = MLPClassifier(
...
def evaluate_model(name, model, Xtr, ytr, Xte, yte, print_details=True):
    ...
    # Probabilidades para AUC (si el modelo las soporta)
...
# Importancias del RF (interpretabilidad)
...
comparison = pd.DataFrame([rf_metrics, mlp_metrics])[
